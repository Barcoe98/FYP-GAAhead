...
const EditDiaryEntryPage = () => {
  ...
  useEffect(() => {
    ...
    setTotalCalories(entry?.ttlCal)
    setLitresWater(entry?.litresWater)
    setHrsSleep(entry?.hrsSleep)

    setBreakfast(entry?.breakfast)
    setBreakfastCal(entry?.breakfastCal)
    setLunch(entry?.lunch)
    setLunchCal(entry?.lunchCal)
    setDinner(entry?.dinner)
    setDinnerCal(entry?.dinnerCal)
    setSupper(entry?.lunch)
    setSupperCal(entry?.supper)
    setSnacks(entry?.snacks)
    setSnacksCal(entry?.snacksCal)

  }, [currentUser?.uid, entry?.breakfast, entry?.breakfastCal, entry?.date, entry?.dinner, entry?.dinnerCal, entry?.hrsSleep, entry?.litresWater, entry?.lunch, entry?.lunchCal, entry?.snacks, entry?.snacksCal, entry?.steps, entry?.supper, entry?.title, entry?.ttlCal, id]);
